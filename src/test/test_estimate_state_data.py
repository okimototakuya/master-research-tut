...
class TestEstimateStateData(unittest.TestCase):
    ...
    def test_estimate_state_data_fitting_rate_transition_matrix_between_original_and_pca(self):
        '''
        現データとPCA後データとの、遷移行列の一致率を算出
        '''
        for i in range(10):
            dict_param_original = ap3.estimate_state_data(   # 主成分分析をせずに、隠れマルコフモデルを適用する場合
                    input_df_averaged = self.df_averaged.drop('time', axis=1),
            ...
            #print(dict_param_original['遷移行列'])
            #print(dict_param_pca['遷移行列'])
            #bool_fit = 1 if dict_param_original['遷移行列'] == dict_param_pca['遷移行列'] else 0
            #print('試行{i}: 遷移行列の一致率:{bool_fit}'.format(i=i+1, bool_fit=bool_fit))
            #np.testing.assert_array_almost_equal(dict_param_original['遷移行列'], dict_param_pca['遷移行列'])
            print('試行{i}:'.format(i=i+1))
            print(dict_param_original['遷移行列'] - dict_param_pca['遷移行列'])
